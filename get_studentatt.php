...
if(!empty($_POST["classid1"])) 
{
 ...
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {?>
  <p> <?php echo htmlentities($row['SubjectName']); ?><input type="text"  name="marks[]" value="" class="form-control" required="" placeholder="Enter attendance" autocomplete="off"></p>
...
$id=$dta[0];
$id1=$dta[1];
 $query = $dbh->prepare("SELECT StudentId,ClassId FROM tblattendance1 WHERE StudentId=:id1 and ClassId=:id ");
